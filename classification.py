  ...
  """
    ...
    def __init__(self, X, y, pretrained_weights=None):
        ...
        if X is None or y is None:
            raise ValueError("Training datasets X and y can't be `None`.")
        if not isinstance(X, np.ndarray) or not isinstance(y, np.ndarray):
            raise ValueError("Training datasets must be " f"of type `numpy.ndarray`.")
        ...
        if pretrained_weights is not None:
            ...
            if pretrained_weights.shape[0] != self.n_features + 1:
                ...
                try:
                    pretrained_weights = pretrained_weights.reshape(
                        self.n_features + 1, 1
                    )
                except ValueError:
                    raise ValueError(
    ...
    """
        return (1.0 / self.n_samples) * np.sum((self.y - preds) ** 2)

    def _mse_grad(self, preds):
        """
    evaluates the gradient of mse function for predictions.
    """
        return (2.0 / self.n_samples) * (
            self.X.T @ ((preds - self.y) * self._sigmoid_prime(preds))
        )
    ...
    """
        return (-1.0 / self.n_samples) * np.sum(
            np.nan_to_num(self.y * np.log(preds) + (1 - self.y) * np.log(1 - preds))
        )
    ...
    """
        return (1.0 / self.n_samples) * (self.X.T @ (preds - self.y))
    ...
    """
        return (1.0 / self.n_samples) * np.sum(self.y == (preds > 0.5))
    ...
    """
        ...
        if (
            ...
            and np.sum(self.X[:, 0] != np.ones((self.n_samples,), dtype=np.float32))
            != 0
        ):
            self.X = np.concatenate(
                (np.ones((self.n_samples, 1), dtype=np.float32), self.X), axis=1
            )
        ...
        for i in range(epochs):
            ...
            for metric in metrics:
                self.metric_scores[metric] = np.round(
                    self._calc_metric(metric, preds), 4
                )
    ...
    """
        ...
        if (
            ...
            and np.sum(X_test[:, 0] != np.ones((X_test.shape[0],), dtype=np.float32))
            != 0
        ):
