...
def plot_me(X, gauss1, gauss2):
    ...
    y = np.linspace(-10.0, 10.0, num=1000)
...
def init():
    ...
    X = np.concatenate(
        (norm(-4.0, 3.0).rvs(10).reshape(-1, 1), norm(5.0, 1.0).rvs(10).reshape(-1, 1)),
        axis=0,
    ).reshape(-1,)
    mu1, sig1, mu2, sig2 = (
        np.random.rand(),
...
def animate(frame):
    ...
    mu1, sig1, mu2, sig2, gauss1, gauss2 = step(
        X, gauss1, gauss2, mu1=mu1, sig1=sig1, mu2=mu2, sig2=sig2
    )
    plot_me(X, gauss1, gauss2)
    return plot1, plot2, sc1, sc2
...
if __name__ == "__main__":
    ...
    fig, ax = plt.subplots()
    anim = FuncAnimation(
        fig, animate, init_func=init, blit=True, frames=50, interval=200
    )
