...
class Dataset:
    ...
    def gen_dataset(self):
        ...
        self.dataset[:, 0] = np.arange(
            start=1, stop=self.nb_samples + 1, dtype=np.float32
        )
    ...
    def get_dataset(self):
        self.dataset = np.loadtxt(self.path, dtype=np.float32, delimiter=",").reshape(
            -1, 2
        )
        return self.dataset
...
class HistTransformer:
    ...
    def _discretize_dataset(self, usecol):
        ...
        self._bins = np.linspace(
            self.__low, self.__high, num=self.bins + 1, endpoint=True, dtype=np.float32
        ).reshape(-1,)
