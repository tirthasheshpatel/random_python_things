#############################################
# Not mine : Code is taken from https://github.com/HackerPoet/GoblinEscape/blob/master/GoblinEscape.py
# Thanks to code parade for making the code available for free!!
#############################################
...
def clear():
    ...
    window.fill((0, 80, 0))
    pygame.draw.circle(
        window, (0, 0, 128), (int(width / 2), int(height / 2)), int(radius * 1.00), 0
    )
    pygame.draw.circle(
        window,
        (200, 200, 200),
        (int(width / 2), int(height / 2)),
        int(radius * radius_mult),
        1,
    )
...
def redraw(draw_text=False, win=False):
    ...
    pygame.draw.circle(
        window, (255, 255, 255), (int(width / 2 + boatx), int(height / 2 + boaty)), 6, 2
    )
    pygame.draw.circle(
        window,
        (255, 0, 0),
        (
            int(width / 2 + radius * math.cos(goblin)),
            int(height / 2 + radius * math.sin(goblin)),
        ),
        6,
        0,
    )
...
def updateGoblin():
    ...
    if abs(diff) * radius <= gspeed * speed_mult:
        ...
        goblin += (
            gspeed * speed_mult / radius
            if diff > 0.0
            else -gspeed * speed_mult / radius
        )
    if goblin < math.pi:
        goblin += math.pi * 2.0
    if goblin > math.pi:
