...
class GridProblem(Problem):
    ...
    def __init__(self, initial=(0, 0), goal=(9, 9), obstacles=(), **kwds):
        Problem.__init__(
            ...
            initial=initial,
            goal=goal,
            obstacles=set(obstacles) - {initial, goal},
            **kwds,
        )
...
class AnimateProblem(GridProblem):
    ...
    def step(self, frame):
        ...
        self.ax.axis("off")
        self.ax.axis("equal")
        self.sc1 = self.ax.scatter(
            *transpose(self.obstacles), marker="s", color="darkgrey"
        )
        self.sc2 = self.ax.scatter(
            *transpose(list(self.reached)), 1 ** 2, marker=".", c="blue"
        )
        self.sc3 = self.ax.scatter(*transpose(self.solution), marker="s", c="blue")
        self.sc4 = self.ax.scatter(
            *transpose([node.state]), 9 ** 2, marker="8", c="yellow"
        )
        self.sc5 = self.ax.scatter(
            *transpose([self.initial]), 9 ** 2, marker="D", c="green"
        )
        self.sc6 = self.ax.scatter(*transpose([self.goal]), 9 ** 2, marker="8", c="red")
        plt.title(
            f"Explored: {len(self.reached)}, Path Cost: {node.path_cost}\nSolver: {self.solver}"
        )
        return self.sc1, self.sc2, self.sc3, self.sc4, self.sc5, self.sc6
    ...
    def run(self, frames=120):
        ...
        """
        anim = FuncAnimation(
            self.fig, self.step, blit=False, interval=1000, frames=frames
        )
